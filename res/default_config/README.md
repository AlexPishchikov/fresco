...
- `fresco_calculate_time_function`
    ...
    **Описание:** Функция, вычисляющая количество времени на размышления. Переменная `rating` отвечает за количество плюсиков студента.
...
- `roulette_spin_function_horisontal`

   **Тип:** `string`

    **Описание:** Функция, отвечающая за скорость движения кнопки по горизонтали. Можно использовать переменные `t` и `i`(последняя отвечает за номер кнопки).

- `roulette_spin_function_vertical`

   **Тип:** `string`

    **Описание:** Аналогично `spin_function_horisontal`, только по вертикали.
